    ...
    <div>
      <div className="filters-bar">
        <button
          ...
          style={{ padding: '0.35rem 0.5rem', fontSize: '0.8rem', border: '1px solid var(--border-color)', borderRadius: '4px', background: 'var(--card-bg)', cursor: 'pointer' }}
        >◀</button>
        <span style={{ fontWeight: 700, color: 'var(--accent-color)', fontSize: '0.95rem', fontFamily: 'Outfit' }}>
        ...
        <button
          ...
          style={{ padding: '0.35rem 0.7rem', fontSize: '0.75rem', border: '1px solid var(--border-color)', borderRadius: '4px', background: 'var(--card-bg)', cursor: 'pointer', fontWeight: 600 }}
        >Today</button>
        ...
        <div className="side-panel-overlay" onClick={() => setSelectedDay(null)}>
          ...
          <div className="side-panel-body" onClick={e => e.stopPropagation()}>
            {selectedHearings.length === 0 ? (
              <p style={{ color: 'var(--text-muted)' }}>No hearings on this day</p>
            ) : (
              selectedHearings.map(c => (
                <div
                  key={c.id}
                  style={{
                    padding: '0.75rem', border: '1px solid var(--border-light)', borderRadius: '6px',
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.3rem', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', background: 'var(--card-hover-bg)', padding: '0.1rem 0.3rem', borderRadius: '3px' }}>{c.forum}</span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>{c.present_status || 'Pending'}</span>
                    <button
                      onClick={(e) => { e.stopPropagation(); handleResync(c.id); }}
                      style={{
                        marginLeft: 'auto', fontSize: '0.6rem', padding: '0.15rem 0.3rem',
                        background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '3px',
                        cursor: 'pointer', color: 'var(--accent-color)', fontWeight: 600,
                      }}
                    >🔄 Re-sync</button>
                  </div>
                </div>
